...
echo "4. nothing is installed into a project"
if [ -f "$SETTINGS" ] && grep -q 'forge-monitor\|emit.sh' "$SETTINGS" 2>/dev/null; then
  ok "hooks are registered in user settings ($SETTINGS)"
else
  printf '  note  not installed in %s yet (run install.sh)\n' "$SETTINGS"
fi
found=$(find . -maxdepth 4 -path ./node_modules -prune -o -name settings.json -print0 2>/dev/null \
        | xargs -0 -r grep -l 'forge-monitor\|monitor/hooks/emit.sh' 2>/dev/null | grep '\.claude' || true)
[ -z "$found" ] && ok "no project .claude/settings.json references the monitor" \
                || no "monitor leaked into a project: $found"

echo "5. no session-facing surface exists for it"
...
echo "6. the collector folds events into state"
...
echo "7. re-reading is incremental, not cumulative"
python3 "$MON_DIR/collector.py" --once >/dev/null 2>&1 || true
r=$(probe waiting verify-s1)
...
echo "8. the file sink is a drop-in replacement for github"

...
# Two layouts must both work: the repo (capabilities/scaffold/scaffold.sh, with
# stacks/ two levels up) and a projected skill (skills/scaffold/scaffold.sh,
# with stacks/ copied in beside it). Look for the stacks tree, do not assume it.
_here="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
if [ -d "$_here/stacks" ]; then
  FORGE_ROOT="$_here"
elif [ -d "$_here/../../stacks" ]; then
  FORGE_ROOT="$(cd "$_here/../.." && pwd)"
else
  echo "scaffold: cannot find the stacks/ tree from $_here - the install is incomplete" >&2
  exit 1
fi
